...
class GBParser:
    ...
    def parse_feed(self, response: requests.Response):
        soup = bs4.BeautifulSoup(response.text, "html.parser")  # ''lxml')
    ...
    def parse_post(self, response: requests.Response):
        ...
        try:
            ...
            data = {
                ...
                # Первое изображение материала (Ссылка)
                "first_img": soup.find('div', attrs={"itemprop": "articleBody"}).find("img").attrs.get("src"),
                # Дата публикации
                "publish_date": soup.find("time", attrs={"itemprop": "datePublished"}).get("datetime"),
                "author": {
            ...
            print(f"Exception during post parse {response.url}: {e}")
...
if __name__ == '__main__':
    ...
    parser = GBParser("https://gb.ru/posts", db)
    parser.run()
